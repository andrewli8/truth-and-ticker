...
- Deep-dive chart now labels the market reaction at the playhead (signed %, coloured
  up/down with a bg halo for legibility) so the quote's effect reads ON the line, not only
  in the side card — the core "when he posts, the market moves" relationship. Positioned
  below the dot near the top edge to avoid the current-price readout. MarketChart gained an
  optional `reactionPct` prop; covered by tests (renders value, direction, absent on null).
- Master-timeline overview now labels the selected event's reaction on the chart beside its
  marker dot (signed %, coloured up/down with a bg halo), mirroring the deep-dive's playhead
  label — so the move and its data point read together, not split from the EventDetail panel.
  Nudged in from the right edge / top so it never clips; omitted when the reaction is null.
  Covered by tests (label renders the formatted reaction; tracks the selection).
- Real-browser E2E guards (e2e/regression.spec.ts) now lock the two on-chart reaction
...
- Deep-dive reaction label is now anchored at the announcement's own data point (a hollow
  event-ring marker), not the scroll-driven playhead — so the "+0.88%" stays pinned to the
...
- Locked MarketChart's no-eventISO fallback branch with a test: with reactionPct set but no
  eventISO, the reaction-callout still renders (playhead-anchored, legacy behaviour) and no
...
1. The POC (now a linked page) has no Windows High Contrast / forced-colors handling,
   unlike its sibling chart surfaces. Evidence: `grep -ic forced-colors src/poc/poc.css`
   returns 0, whereas src/components/CategoryBand.module.css and
   src/components/ReactionSpread.module.css both ship `@media (forced-colors: active)`
   fallbacks. The POC's text-shadow readout halo and custom-property accents are dropped
   in forced-colors, and the area fill / grain provide no system-colour fallback.
...
2. The POC's accessibility posture (keyboard slider scrubbing with arrow/Home/End, a
   ...
   Evidence: `grep -ic poc docs/ACCESSIBILITY.md` returns 0, though the doc covers the
   main app's Keyboard/Motion posture (docs/ACCESSIBILITY.md:6, :29) and the POC is now
   linked from the Outro.
